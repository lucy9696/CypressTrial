...
Cypress.Commands.add('signUpEmail',(first_name,last_name,email,password,country,country_code) => {
    cy.get('form').within(($form) =>{  
        //accessing within the form unlike normal sign ins, it searches for elements only within the form and not the entire document
        cy.get('input[name = "user[first_name]"]').type(first_name)
        cy.get('input[name = "user[last_name]"]').type(last_name)
        cy.get('input[name = "user[email]"]').type(email)
        cy.get('input[name = "user[password]"]').type(password)
        cy.get('select').select(country).should('have.value', country_code)
...
Cypress.Commands.add('deleteUser',(email) => {
    cy.get('form').within(($form) =>{  
        //accessing within the form unlike normal sign ins, it searches for elements only within the form and not the entire document
        cy.get('input[name = "user[email]"]').type("anet@steadyhq.com")
        cy.get('input[name = "user[password]"]').type("1!Superman")
        cy.root().submit()})
        cy.contains('Users').click()
        cy.wait(2000)
        cy.get('input[name = "search[for]"]').type(email)
        cy.wait(2000)
